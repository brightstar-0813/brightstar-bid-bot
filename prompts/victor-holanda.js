...
Rewrite Victor Barros de Holanda's resume for ONE target job description using the verified profile, career history, and company context supplied below.

Your highest priority is not generic Salesforce wording. It is this: a recruiter reading the resume must immediately think "this candidate has already done work very similar to what we need" — because every important requirement in the JD that Victor genuinely supports is visible in the Skills section AND proven inside Professional Experience with a real project, his personal responsibility, and an outcome.
...
Ignore any stored memory, earlier conversation, or personal detail about this candidate that is not written in this prompt. This prompt is the complete and only source of truth about Victor. If you "remember" a fact about him that is not below, it does not exist.
...
GATE 1A — THE TWO MOST RECENT ROLES MUST PROVE THE MUST-HAVES. EPAM Systems and Endava each carry AT LEAST 3 bullets that name the JD's required skills and describe real work with them — not one bullet listing all of them, but three separate bullets spread across different workflows. The skills table alone is worthless: a recruiter checks whether the recent roles actually did the work. If Data Cloud and Agentforce are required and neither appears in a EPAM Systems bullet, the resume has failed no matter how good the skills row looks.
Priority order inside those bullets: the skills the JD marks MUST have first, then the rest of the required list. A required skill that is already common in Victor's history (Apex, Service Cloud) does NOT satisfy this gate on its own — the scarce, newly required products must each appear in at least one recent-role bullet.
Where a product postdates a role, put it in EPAM Systems rather than Endava, and give Endava the era-appropriate members of the required list instead.
...
GATE 7 — HOLD THE FIXED FACTS. Employers, dates, locations, titles, education, and the certification list stay exactly as given, in the given order. Experience framing stays 10+ years in technology including 7+ years of Salesforce delivery.
...
Then classify every Tier 1 and Tier 2 requirement against Victor's real background:
...
Tier 0 is NOT run through this classification and NOT run through the COVERAGE LADDER. Whether or not the material below mentions a Tier 0 skill, it goes into the resume as a skill Victor has. See TIER 0 TREATMENT.
...
- what Victor personally designed, built, configured, integrated, migrated, secured, tested, deployed, debugged, or documented,
...
- EPAM Systems (current role): roughly 60-70% of the strongest JD-aligned themes.
- Endava: deep senior Salesforce developer and automation-engineer themes.
- BairesDev: multi-year Salesforce developer and integration engineer ownership.
- Accenture: software engineer intern / junior CRM developer foundation only. Never convert this into a senior architecture role.
...
Write every Tier 0 skill as a skill Victor has — flat, in the same voice as every other skill, with no hedge, no parenthetical softener, and no distancing verb. Do not write "exposure to Data Cloud", "working knowledge of Agentforce", "familiar with", "evaluated", "designed toward", "proof of concept", "(basic)", or "learning". A hedged must-have reads as an admission and fails the screen exactly as an omission does. Either the skill is on the resume as a skill, or the application is not worth sending.

Give Tier 0 experience bullets the same specificity demanded everywhere else in this prompt: the real employer's real business process, exact feature names, what Victor personally built, and the result. A vague Tier 0 bullet is worse than none — it announces that the keyword was bolted on. If a Tier 0 bullet could be pasted into any other candidate's resume unchanged, rewrite it.
...
- Agentforce, Einstein Copilot, Prompt Builder, Agent Builder, Einstein Trust Layer: late 2024 onward — EPAM Systems ONLY.
- Salesforce Data Cloud (and its Genie / Customer 360 Audiences lineage): 2023 onward — EPAM Systems; Endava only in late-tenure bullets.
- Salesforce-native Document Generation: 2023 onward — EPAM Systems. Conga Composer and Nintex Drawloop date to the early 2010s and may appear in older roles where DocGen is the requirement.
- Public Sector Solutions, OmniStudio: 2021 onward — EPAM Systems and Endava.
- CRM Analytics: use that name only for 2021 onward; for earlier work write Einstein Analytics or Wave, and only from 2016.
- Lightning Web Components, Salesforce DX, Salesforce CLI, Gearset, Copado, Flow Builder: 2019 onward — EPAM Systems, Endava, and later BairesDev years.
- Lightning Experience and Aura: 2016 onward.
- The earliest roles (Accenture internship / junior CRM years) use only what existed then: Salesforce Classic, Visualforce, Apex triggers, workflow rules, approval processes, reports and dashboards, Data Loader, Change Sets.
...
RUNG 5 — FOUNDATION EVIDENCE. An earlier role or adjacent domain proves the underlying competence even though the exact tool differs — the early platform work at Accenture supports "relational data modeling and declarative platform fundamentals"; global digital engineering, automation, nearshore product engineering, and consulting delivery supports high-volume operational and customer-facing contexts.
...
A skills CATEGORY may be named after the JD's theme even when its listed items are Victor's real tools. Naming the category "Integration & Middleware" when the JD says MuleSoft is legitimate framing; adding "MuleSoft" to the list is not.
...
- Contact phone in the JSON must be exactly "+55 81 9573 5610". Never substitute another number.
...
THE INTERVIEW TEST — apply to every Tier 1, Tier 2, and Tier 3 line before it ships: could Victor answer three specific follow-up questions about this from real memory, and would it survive a reference check and a technical screen? If not, it belongs on Rung 6.
Tier 0 lines are written to clear the screen and are exempt from this test by design. That exemption comes with an obligation: write them concretely enough that they describe a real, coherent piece of work — specific features, a specific workflow, a specific outcome — so they hold together as a body of work Victor can prepare against before the technical conversation. Vague Tier 0 padding fails both the screen and the interview.
...

{
  ...
  "profile": "Senior Salesforce Engineer with 10+ years in technology and 7+ years delivering Salesforce solutions across global digital engineering, automation-led CRM delivery, nearshore product engineering, and consulting.",
  "education": [
      {
  ...
  "experience": [
    {
      ...
      "location": "Newtown, Pennsylvania, United States | Remote",
...
- linkedin must be exactly: "linkedin": "https://www.linkedin.com/in/victorstardev1996"
...
- Domain language aligned to Victor's real company contexts plus JD themes
...
- State experience as 10+ years in technology including 7+ years of Salesforce delivery (do not inflate beyond this)
...
- Mention the relevant business domain when supported (global digital engineering, automation-led CRM, nearshore product engineering, consulting)
...
It lists EVERY Salesforce cloud and product the JD names, in the JD's order of emphasis, BEFORE Victor's other clouds. If the JD says "Primary Skill: Service Cloud / Required: Data Cloud, Agentforce, PSS", the row reads:
...
- Include relevant technologies already in Victor's background even when the JD is silent, but keep them subordinate to JD priorities.
- Do not paste the JD's skill list verbatim as a block and do not keyword-dump. Tier 0 terms belong here; the surrounding stack must still be Victor's own.
...
- Use the category names from the approved list above. Do not reproduce the generic category set from Victor's existing resume ("Platform Administration & Security", "Integration & Data", "Analytics", "Technical Leadership") — those are the untailored source, and repeating them is a sign no tailoring happened.
...
- EPAM Systems (Sep 2024 - Present): 10-12 bullets
- Endava (Jul 2022 - Jun 2024): 9-11 bullets
- BairesDev (Dec 2018 - May 2022): 8-10 bullets
- Accenture (May 2016 - Oct 2018): 3-5 bullets

Set "location"
Set "location" for each role to the exact "City, State, Country | Mode" string from FIXED COMPANY HISTORY.
NEVER append an employment type to the location line. Full-Time, Part-Time, Contract, Contract-to-Hire, Temporary, Intern, Freelance, W2, C2C and every variant are banned from the location line only. Work mode (Hybrid / Remote / On-Site) stays. The Accenture title must stay exactly "Software Engineer Intern / Junior CRM Developer" — Intern and Junior are required and must never be stripped.
...
Action verb + specific system, workflow, or business process + exact technology + what Victor personally owned + engineering or business result.
...
5. What did Victor personally design, build, configure, integrate, test, migrate, secure, deploy, debug, or document?
...
DEPTH TARGETS
- EPAM Systems / Endava / BairesDev: one long sentence each, roughly 150-260 characters, naming a concrete Salesforce artifact.
- Accenture: roughly 100-170 characters, framed as internship / junior CRM foundation work.
...
When the JD names a technology, show depth instead of the bare name, wherever Victor's history supports it:
...
Use only features consistent with Victor's actual work and with the technology available during that employment period.

==================================================
DEPTH PLAYBOOK — WHEN THE JD LEANS ON AN AREA
==================================================

INTEGRATION DEPTH. When integrations matter, make them one of the strongest areas of the resume, and for each integration bullet identify as many of these as the history supports: source system, target system, the business data exchanged, the integration technology, the authentication method, the payload format, the sync pattern, the error-handling pattern, and what Victor personally owned. Real pairings for his history: Salesforce to ERP and billing platforms, to field-service dispatch and scheduling, to student-information and campus systems, to distributor and inventory systems, to marketing automation, and to data warehouses through MuleSoft.
...
NON-SALESFORCE ROLES. Accenture keeps its internship / junior CRM identity. Connect it forward through requirements support, configuration fundamentals, testing, and documentation. Never restyle it as a senior Salesforce architecture role.
...
Surface real transferable domain experience when the JD belongs to an industry Victor actually served:
- Global digital engineering and AI transformation: EPAM Systems — custom software, product and platform engineering for large enterprises; Salesforce engineering on transformation programs.
- Digital transformation and continuous delivery: Endava — engineering, automation, cloud, and industry solutions; Salesforce development with test and release automation.
- Nearshore product engineering: BairesDev — high-performing software delivery for startups through Fortune Global 500; Salesforce development and integrations.
- Consulting and systems integration: Accenture — junior CRM and software engineering internship foundation in Kyoto.

Outside of Tier 0,Outside of Tier 0, do not claim industry regulations, clinical systems, or platforms with no factual basis. Never claim a government agency, program, or contract Victor did not work on, and never claim a clearance.

==================================================
COMPANY CONTEXT (use the real business of each employer)
==================================================

EPAM Systems (Newtown, Pennsylvania, United States | Remote | current):
Global AI transformation engineering and integrated consulting firm, 10,001+ employees, IT Services and IT Consulting. Founded 1993. Headquarters Newtown, PA.
Victor's work: senior Salesforce engineering — multi-cloud CRM delivery, Apex and Lightning Web Components, Data Cloud and Agentforce where era-safe, integrations, data architecture, CI/CD, automation, and production support for enterprise clients.

Endava (Bucharest, Romania | Remote):
Digital transformation engineering firm, 10,001+ employees. Specialties: digital strategy, continuous delivery and DevOps, cloud, distributed agile, automation.
Victor's work: senior Salesforce developer and automation engineer — Apex, LWC, Flow, integration work, automated testing and release pipelines, Service Cloud and Sales Cloud delivery, stakeholder collaboration.

BairesDev (Recife, Pernambuco, Brazil | Hybrid):
Nearshore software development firm, 1,001-5,000 employees. Headquarters San Francisco. Specialties include CRM, cloud, automation, and staff augmentation for global clients.
Victor's work: Salesforce developer and integration engineer — Apex, APIs, middleware patterns where historically accurate, data migration, declarative automation, and multi-year platform ownership. Longest mid-career tenure — show sustained ownership.

Accenture (Kyoto, Kyoto Prefecture, Japan | On-site | internship / junior):
Global professional services and technology firm, 10,001+ employees.
Victor's work: software engineer intern / junior CRM developer — requirements support, Salesforce configuration fundamentals, Apex and Visualforce basics, testing, documentation. Keep clearly junior.
==================================================
CAREER PROGRESSION AND ANTI-OVER-TAILORING
==================================================

- EPAM Systems carries the strongest JD alignment and senior-engineering voice.
- Endava is the automation-heavy senior developer role.
- BairesDev shows multi-year hands-on Salesforce development and integrations.
- Accenture shows early internship / junior CRM foundation only.
- Never place modern tooling in a historical role before it plausibly existed (no Data Cloud or Agentforce in 2016-2018 Accenture years).
- Do not make every role contain every JD keyword, do not copy JD sentences, and do not recreate the target company's product as if Victor already built it.
...
Technical specificity outranks numbers. Include at most 1-2 modest, defensible metric-style results in the EPAM Systems role and optionally one in Endava or BairesDev, only when natural. Prefer qualitative outcomes: reduced manual processing, fewer recurring defects, cleaner releases, faster troubleshooting, improved data accuracy, stronger access control, better integration stability. Never claim revenue impact, team size, user counts, or large percentage gains.
...
Experience framing: 10+ years of overall technology experience, including 7+ years delivering Salesforce solutions across global digital engineering, automation-led CRM delivery, nearshore product engineering, and consulting.

Education:
Kyoto University | Bachelor of Engineering in Computer Science | Kyoto, Japan | Apr 2014 - Mar 2018
Grade: 3.3 / 4.0

Verified Certifications (this list is complete and closed — reproduce in this exact order):
- Salesforce Certified Platform Data Architect — Issued Oct 2025
- Salesforce Certified Agentforce Specialist — Issued May 2025
- Salesforce Certified Data 360 Consultant — Issued Sep 2024
- Salesforce Certified Platform Developer — Issued Jul 2023
- Salesforce Certified Platform Administrator — Issued Apr 2023
- Salesforce Certified Platform Foundations — Issued Apr 2023
...
EPAM Systems (Newtown, Pennsylvania, United States | Remote) — default title: Senior Salesforce Engineer | Sep 2024 - Present

Endava (Bucharest, Romania | Remote) — default title: Senior Salesforce Developer & Automation Engineer | Jul 2022 - Jun 2024

BairesDev (Recife, Pernambuco, Brazil | Hybrid) — default title: Salesforce Developer | Integration Engineer | Dec 2018 - May 2022

Accenture (Kyoto, Kyoto Prefecture, Japan | On-site) — default title: Software Engineer Intern / Junior CRM Developer | May 2016 - Oct 2018
==================================================
EXISTING TECHNICAL COVERAGE (draw selectively; never dump all of it)
==================================================

Salesforce clouds and products Victor works across:
...
0g2. RECENT-ROLE PROOF — count, per role, how many EPAM Systems bullets and how many Endava bullets name a required skill. Both counts must be 3 or more, and every scarce required product must appear in at least one recent-role bullet. If either count is short, add bullets until it is met before returning.
0h. NO EMPLOYMENT TYPE IN LOCATION — search every experience location for "Full-Time", "Full Time", "Part-Time", "Contract", "Temporary", "Intern", "Freelance", "W2", "C2C" and remove it from location only. Each location reads "City, State, Country | Mode" and nothing more. Keep title exactly "Software Engineer Intern / Junior CRM Developer" for Accenture.
...
0c. ERA CHECK — no Tier 0 or modern skill sits in a role that predates it. Agentforce and Data Cloud appear at EPAM Systems only.
...
4b. TAILORING PROOF — the skills table and the EPAM Systems bullets are visibly specific to THIS JD, not a restatement of Victor's existing resume.
5. RESPONSIBILITY — every bullet shows what Victor personally did.
...
11. INTERVIEW TEST — every Tier 1/2/3 bullet is one Victor could explain from memory. Tier 0 bullets are exempt from this check (see TIER 0 TREATMENT); they must instead be concrete and internally coherent. Never delete a Tier 0 bullet for failing this test — rewrite it with sharper detail.
