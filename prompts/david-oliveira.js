...
Your highest priority is not generic Salesforce wording. It is this: a recruiter reading the resume must immediately think "this candidate has already done work very similar to what we need" — because every important requirement in the JD that David genuinely supports is visible in the Skills section AND proven inside Professional Experience with a real project, his personal responsibility, and an outcome.
...
GATE 1A — THE TWO MOST RECENT ROLES MUST PROVE THE MUST-HAVES. Intrado and S&P Global each carry AT LEAST 3 bullets that name the JD's required skills and describe real work with them — not one bullet listing all of them, but three separate bullets spread across different workflows. The skills table alone is worthless: a recruiter checks whether the recent roles actually did the work. If Data Cloud and Agentforce are required and neither appears in a Intrado bullet, the resume has failed no matter how good the skills row looks.
Priority order inside those bullets: the skills the JD marks MUST have first, then the rest of the required list. A required skill that is already common in David's history (Apex, Service Cloud) does NOT satisfy this gate on its own — the scarce, newly required products must each appear in at least one recent-role bullet.
Where a product postdates a role, put it in Intrado rather than S&P Global, and give S&P Global the era-appropriate members of the required list instead.
...
GATE 7 — HOLD THE FIXED FACTS. Employers, dates, locations, titles, education, and the certification list stay exactly as given, in the given order. Experience framing stays 11+ years in technology including 8+ years of Salesforce delivery.
...
Then classify every Tier 1 and Tier 2 requirement against David's real background:
...
Tier 0 is NOT run through this classification and NOT run through the COVERAGE LADDER. Whether or not the material below mentions a Tier 0 skill, it goes into the resume as a skill David has. See TIER 0 TREATMENT.
...
- what David personally designed, built, configured, integrated, migrated, secured, tested, deployed, debugged, or documented,
...
- Intrado (current role): roughly 60-70% of the strongest JD-aligned themes.
- S&P Global: deep enterprise Salesforce delivery in financial data and analytics contexts.
- AI/R Everymind: Salesforce developer tenure across AI-enabled CRM consulting and Salesforce Industries.
- Pismo: Salesforce Administrator foundation in payments and digital banking platforms.
- Concrete Solutions: software engineer internship only — never convert into a Salesforce architecture role.
...
Write every Tier 0 skill as a skill David has — flat, in the same voice as every other skill, with no hedge, no parenthetical softener, and no distancing verb. Do not write "exposure to Data Cloud", "working knowledge of Agentforce", "familiar with", "evaluated", "designed toward", "proof of concept", "(basic)", or "learning". A hedged must-have reads as an admission and fails the screen exactly as an omission does. Either the skill is on the resume as a skill, or the application is not worth sending.

Give Tier 0 experience bullets the same specificity demanded everywhere else in this prompt: the real employer's real business process, exact feature names, what David personally built, and the result. A vague Tier 0 bullet is worse than none — it announces that the keyword was bolted on. If a Tier 0 bullet could be pasted into any other candidate's resume unchanged, rewrite it.
...
- Agentforce, Einstein Copilot, Prompt Builder, Agent Builder, Einstein Trust Layer: late 2024 onward — Intrado ONLY.
- Salesforce Data Cloud (and its Genie / Customer 360 Audiences lineage): 2023 onward — Intrado; S&P Global only in late-tenure bullets.
- Salesforce-native Document Generation: 2023 onward — Intrado. Conga Composer and Nintex Drawloop date to the early 2010s and may appear in older roles where DocGen is the requirement.
- Public Sector Solutions, OmniStudio: 2021 onward — Intrado and S&P Global.
- CRM Analytics: use that name only for 2021 onward; for earlier work write Einstein Analytics or Wave, and only from 2016.
- Lightning Web Components, Salesforce DX, Salesforce CLI, Gearset, Copado, Flow Builder: 2019 onward — Intrado, S&P Global, and AI/R Everymind.
- Lightning Experience and Aura: 2016 onward.
- The earliest roles (Concrete Solutions internship and early Pismo administrator years) use only what existed then: Salesforce Classic, Visualforce, Apex triggers, workflow rules, approval processes, reports and dashboards, Data Loader, Change Sets.
...
RUNG 5 — FOUNDATION EVIDENCE. An earlier role or adjacent domain proves the underlying competence even though the exact tool differs — the early platform work at Concrete Solutions supports "relational data modeling and declarative platform fundamentals"; public safety, financial services, AI CRM consulting, and payments delivery supports high-volume operational and customer-facing contexts.
...
A skills CATEGORY may be named after the JD's theme even when its listed items are David's real tools. Naming the category "Integration & Middleware" when the JD says MuleSoft is legitimate framing; adding "MuleSoft" to the list is not.
...
- Contact phone in the JSON must be exactly "+55 61 8212 1297". Never substitute another number.
...
THE INTERVIEW TEST — apply to every Tier 1, Tier 2, and Tier 3 line before it ships: could David answer three specific follow-up questions about this from real memory, and would it survive a reference check and a technical screen? If not, it belongs on Rung 6.
Tier 0 lines are written to clear the screen and are exempt from this test by design. That exemption comes with an obligation: write them concretely enough that they describe a real, coherent piece of work — specific features, a specific workflow, a specific outcome — so they hold together as a body of work David can prepare against before the technical conversation. Vague Tier 0 padding fails both the screen and the interview.
...

{
  ...
  "profile": "Senior Salesforce Engineer with 11+ years in technology and 8+ years delivering Salesforce solutions across public safety communications, financial data platforms, AI-enabled CRM consulting, payments, and digital product teams.",
  "education": [
      {
        "school": "Federal University of Pernambuco",
        "degree": "Bachelor of Science in Computer Science",
        "year": "Mar 2013 - Aug 2017",
...
- linkedin must be exactly: "linkedin": "https://www.linkedin.com/in/davidleandro"
...
- Domain language aligned to David's real company contexts plus JD themes
...
- State experience as 11+ years in technology including 8+ years of Salesforce delivery (do not inflate beyond this)
...
- Mention the relevant business domain when supported (public safety communications, financial data, AI CRM consulting, payments, digital products)
...
It lists EVERY Salesforce cloud and product the JD names, in the JD's order of emphasis, BEFORE David's other clouds. If the JD says "Primary Skill: Service Cloud / Required: Data Cloud, Agentforce, PSS", the row reads:
...
- Include relevant technologies already in David's background even when the JD is silent, but keep them subordinate to JD priorities.
- Do not paste the JD's skill list verbatim as a block and do not keyword-dump. Tier 0 terms belong here; the surrounding stack must still be David's own.
...
- Use the category names from the approved list above. Do not reproduce the generic category set from David's existing resume ("Platform Administration & Security", "Integration & Data", "Analytics", "Technical Leadership") — those are the untailored source, and repeating them is a sign no tailoring happened.
...
- Intrado (Jun 2024 - Present): 10-12 bullets
- S&P Global (Dec 2022 - May 2024): 9-11 bullets
- AI/R Everymind (Oct 2019 - Sep 2022): 8-10 bullets
- Pismo (Sep 2017 - Sep 2019): 6-8 bullets
- Concrete Solutions (Sep 2014 - Aug 2017): 2-4 bullets

Set "location"
Set "location" for each role to the exact "City, State, Country | Mode" string from FIXED COMPANY HISTORY.
NEVER append an employment type to the location line. Full-Time, Part-Time, Contract, Contract-to-Hire, Temporary, Intern, Freelance, W2, C2C and every variant are banned from the location line only. Work mode (Hybrid / Remote / On-Site) stays. The Concrete Solutions title must stay exactly "Software Engineer Intern" — the word Intern is required and must never be stripped.
...
Action verb + specific system, workflow, or business process + exact technology + what David personally owned + engineering or business result.
...
5. What did David personally design, build, configure, integrate, test, migrate, secure, deploy, debug, or document?
...
DEPTH TARGETS
- Intrado / S&P Global / AI/R Everymind: one long sentence each, roughly 150-260 characters, naming a concrete Salesforce artifact.
- Pismo: roughly 110-190 characters, admin and declarative depth.
- Concrete Solutions: roughly 100-170 characters, framed as software engineering internship, not Salesforce.
...
When the JD names a technology, show depth instead of the bare name, wherever David's history supports it:
...
Use only features consistent with David's actual work and with the technology available during that employment period.

==================================================
DEPTH PLAYBOOK — WHEN THE JD LEANS ON AN AREA
==================================================

INTEGRATION DEPTH. When integrations matter, make them one of the strongest areas of the resume, and for each integration bullet identify as many of these as the history supports: source system, target system, the business data exchanged, the integration technology, the authentication method, the payload format, the sync pattern, the error-handling pattern, and what David personally owned. Real pairings for his history: Salesforce to ERP and billing platforms, to field-service dispatch and scheduling, to student-information and campus systems, to distributor and inventory systems, to marketing automation, and to data warehouses through MuleSoft.
...
NON-SALESFORCE ROLES. Concrete Solutions keeps its real software-engineering internship identity. Connect it forward through requirements gathering, testing support, project coordination, and documentation. Never restyle it as a Salesforce role.
...
Surface real transferable domain experience when the JD belongs to an industry David actually served:
- Public safety and emergency communications: Intrado — 911 and emergency response platforms, public safety technology, incident management, operational CRM.
- Financial data and capital markets intelligence: S&P Global — benchmarks, analytics, credit ratings, commodities and ESG intelligence platforms; Salesforce supporting commercial and customer workflows.
- AI-enabled CRM consulting: AI/R Everymind — Salesforce Industries, Tableau CRM, Customer 360, Commerce Cloud, Slack, Data Cloud, and AI implementation/integration.
- Payments and digital banking platforms: Pismo — cloud-native card issuing, digital wallets, and core banking microservices; Salesforce administration supporting customer and operations workflows.
- Digital products consulting: Concrete Solutions (Accenture) — web, mobile, cloud, and agile delivery internship foundation.

Outside of Tier 0,Outside of Tier 0, do not claim industry regulations, clinical systems, or platforms with no factual basis. Never claim a government agency, program, or contract David did not work on, and never claim a clearance.

==================================================
COMPANY CONTEXT (use the real business of each employer)
==================================================

Intrado (Colorado, United States | Remote | current):
Public safety communications and emergency technology company founded 1979, 1,001-5,000 employees.
David's work: senior Salesforce engineering — Service Cloud and multi-cloud CRM delivery, Apex and Lightning Web Components, Data Cloud and AI-adjacent platform work where era-safe, integrations, data architecture, CI/CD, and production support.

S&P Global (New York, New York, United States | Remote):
Financial services and intelligence company, 10,001+ employees. Specialties: analytics, credit ratings, finance, energy and commodities information, ESG, sustainable finance.
David's work: Salesforce developer across commercial and customer platforms — Apex, LWC, Flow, integrations with financial data systems, data quality, reporting, and release support.

AI/R Everymind (São Paulo, Brazil | Hybrid):
IT services and AI/CRM consulting firm, 501-1,000 employees. Specialties: Analytics, SFA, Salesforce Industries, Tableau CRM, CRM, Customer 360, Commerce Cloud, Slack, Data Cloud, implementation and integration.
David's work: Salesforce developer on consulting engagements — Sales Cloud and Service Cloud delivery, Industries solutions, analytics, integrations, and client-facing implementation. Multi-year tenure — show sustained ownership.

Pismo (Recife, Brazil | On-site):
Payments and banking technology platform, 501-1,000 employees. Cloud-native microservices for card issuing, digital wallets, and core banking.
David's work: Salesforce Administrator — configuration, security and access, automation, data management, reports and dashboards, user support for payments and banking operations teams. Prefer admin and declarative depth; Apex only where historically accurate for the period.

Concrete Solutions (Recife, Brazil | On-site | internship):
Digital products firm (part of Accenture), large-scale IT services.
David's work: software engineer intern — web and mobile delivery support, agile ceremonies, testing, documentation. Keep clearly junior; never describe as Salesforce engineering.
==================================================
CAREER PROGRESSION AND ANTI-OVER-TAILORING
==================================================

- Intrado carries the strongest JD alignment and senior-engineering voice.
- S&P Global and AI/R Everymind are the deep developer and consulting roles.
- Pismo shows administrator foundation in a payments platform.
- Concrete Solutions shows early internship foundation only.
- Never place modern tooling in a historical role before it plausibly existed (no Agentforce in 2014-2017 Concrete Solutions; Data Cloud only where era-safe).
- Do not make every role contain every JD keyword, do not copy JD sentences, and do not recreate the target company's product as if David already built it.
...
Technical specificity outranks numbers. Include at most 1-2 modest, defensible metric-style results in the Intrado role and optionally one in S&P Global or AI/R Everymind, only when natural. Prefer qualitative outcomes: reduced manual processing, fewer recurring defects, cleaner releases, faster troubleshooting, improved data accuracy, stronger access control, better integration stability. Never claim revenue impact, team size, user counts, or large percentage gains.
...
Experience framing: 11+ years of overall technology experience, including 8+ years delivering Salesforce solutions across public safety communications, financial data platforms, AI-enabled CRM consulting, payments, and digital product teams.

Education:
Federal University of Pernambuco | Bachelor of Science in Computer Science | Recife, Brazil | Mar 2013 - Aug 2017
Grade: 8.2 / 10
...
SF Cloud experience to draw from selectively: Sales Cloud, Service Cloud, Experience Cloud, Data Cloud, Agentforce, Einstein AI, Nonprofit Cloud, Health Cloud.
...
Intrado (Colorado, United States | Remote) — default title: Senior Salesforce Engineer | Jun 2024 - Present

S&P Global (New York, New York, United States | Remote) — default title: Salesforce Developer | Dec 2022 - May 2024

AI/R Everymind (São Paulo, Brazil | Hybrid) — default title: Salesforce Developer | Oct 2019 - Sep 2022

Pismo (Recife, Brazil | On-site) — default title: Salesforce Administrator | Sep 2017 - Sep 2019

Concrete Solutions (Recife, Brazil | On-site) — default title: Software Engineer Intern | Sep 2014 - Aug 2017
==================================================
EXISTING TECHNICAL COVERAGE (draw selectively; never dump all of it)
==================================================

Salesforce clouds and products David works across:
Sales Cloud, Service Cloud, Experience Cloud, Data Cloud, Agentforce, Einstein AI, Nonprofit Cloud, Health Cloud, CRM Analytics, Salesforce Platform, MuleSoft Integration Cloud.
...
0g2. RECENT-ROLE PROOF — count, per role, how many Intrado bullets and how many S&P Global bullets name a required skill. Both counts must be 3 or more, and every scarce required product must appear in at least one recent-role bullet. If either count is short, add bullets until it is met before returning.
0h. NO EMPLOYMENT TYPE IN LOCATION — search every experience location for "Full-Time", "Full Time", "Part-Time", "Contract", "Temporary", "Intern", "Freelance", "W2", "C2C" and remove it from location only. Each location reads "City, State, Country | Mode" and nothing more. Keep title exactly "Software Engineer Intern" for Concrete Solutions.
...
0c. ERA CHECK — no Tier 0 or modern skill sits in a role that predates it. Agentforce and Data Cloud appear at Intrado only.
...
4b. TAILORING PROOF — the skills table and the Intrado bullets are visibly specific to THIS JD, not a restatement of David's existing resume.
5. RESPONSIBILITY — every bullet shows what David personally did.
...
11. INTERVIEW TEST — every Tier 1/2/3 bullet is one David could explain from memory. Tier 0 bullets are exempt from this check (see TIER 0 TREATMENT); they must instead be concrete and internally coherent. Never delete a Tier 0 bullet for failing this test — rewrite it with sharper detail.
