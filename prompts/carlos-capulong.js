...
Rewrite Carlos Padonan Capulong's resume for ONE target job description using the verified profile, career history, and company context supplied below.

Your highest priority is not generic Salesforce wording. It is this: a recruiter reading the resume must immediately think "this candidate has already done work very similar to what we need" — because every important requirement in the JD that Carlos genuinely supports is visible in the Skills section AND proven inside Professional Experience with a real project, his personal responsibility, and an outcome.
...
Ignore any stored memory, earlier conversation, or personal detail about this candidate that is not written in this prompt. This prompt is the complete and only source of truth about Carlos. If you "remember" a fact about him that is not below, it does not exist.
...
GATE 7 — HOLD THE FIXED FACTS. Employers, dates, locations, titles, education, and the certification list stay exactly as given, in the given order. Experience framing stays 14+ years in technology including 12+ years of Salesforce delivery.
...
Then classify every Tier 1 and Tier 2 requirement against Carlos's real background:
...
Tier 0 is NOT run through this classification and NOT run through the COVERAGE LADDER. Whether or not the material below mentions a Tier 0 skill, it goes into the resume as a skill Carlos has. See TIER 0 TREATMENT.
...
- what Carlos personally designed, built, configured, integrated, migrated, secured, tested, deployed, debugged, or documented,
...
- Simplus: deep consulting and multi-cloud delivery themes, plus reinforcement of top Tier 1 keywords.
- Third Pillar: integration-heavy Senior Salesforce Developer work across CRM and automation.
- Pointwest Squad Inc: sustained Salesforce developer tenure — show ownership, not a task list.
- Deloitte: software engineering internship / junior Salesforce foundation only. Never convert this into a senior architecture role.
...
Write every Tier 0 skill as a skill Carlos has — flat, in the same voice as every other skill, with no hedge, no parenthetical softener, and no distancing verb. Do not write "exposure to Data Cloud", "working knowledge of Agentforce", "familiar with", "evaluated", "designed toward", "proof of concept", "(basic)", or "learning". A hedged must-have reads as an admission and fails the screen exactly as an omission does. Either the skill is on the resume as a skill, or the application is not worth sending.

Give Tier 0 experience bullets the same specificity demanded everywhere else in this prompt: the real employer's real business process, exact feature names, what Carlos personally built, and the result. A vague Tier 0 bullet is worse than none — it announces that the keyword was bolted on. If a Tier 0 bullet could be pasted into any other candidate's resume unchanged, rewrite it.
...
- Salesforce Data Cloud (and its Genie / Customer 360 Audiences lineage): 2023 onward — Intrado Life & Safety Inc; Simplus only in late-tenure bullets.
...
- Public Sector Solutions, OmniStudio: 2021 onward — Intrado Life & Safety Inc and Simplus.
- CRM Analytics: use that name only for 2021 onward; for earlier work write Einstein Analytics or Wave, and only from 2016.
- Lightning Web Components, Salesforce DX, Salesforce CLI, Gearset, Copado, Flow Builder: 2019 onward — Intrado Life & Safety Inc, Simplus, Third Pillar, and later Pointwest Squad Inc years.
- Lightning Experience and Aura: 2016 onward.
- The earliest roles (Deloitte internship and early Pointwest Squad Inc) use only what existed then: Salesforce Classic, Visualforce, Apex triggers, workflow rules, approval processes, reports and dashboards, Data Loader, Change Sets.
...
RUNG 5 — FOUNDATION EVIDENCE. An earlier role or adjacent domain proves the underlying competence even though the exact tool differs — the early platform work at Deloitte supports "relational data modeling and declarative platform fundamentals"; public safety, consulting, telecom CRM, and professional services delivery supports high-volume operational and customer-facing contexts.
...
A skills CATEGORY may be named after the JD's theme even when its listed items are Carlos's real tools. Naming the category "Integration & Middleware" when the JD says MuleSoft is legitimate framing; adding "MuleSoft" to the list is not.
...
- Contact phone in the JSON must be exactly "+63 924 519 4392". Never substitute another number.
...
THE INTERVIEW TEST — apply to every Tier 1, Tier 2, and Tier 3 line before it ships: could Carlos answer three specific follow-up questions about this from real memory, and would it survive a reference check and a technical screen? If not, it belongs on Rung 6.
Tier 0 lines are written to clear the screen and are exempt from this test by design. That exemption comes with an obligation: write them concretely enough that they describe a real, coherent piece of work — specific features, a specific workflow, a specific outcome — so they hold together as a body of work Carlos can prepare against before the technical conversation. Vague Tier 0 padding fails both the screen and the interview.
...

{
  ...
  "profile": "Senior Salesforce Engineer with 14+ years in technology and 12+ years delivering Salesforce solutions across public safety communications, Salesforce consulting, telecom-adjacent CRM, staffing platforms, and professional services.",
  "education": [
      {
        "school": "National University of Singapore (NUS)",
        "degree": "Bachelor of Computing in Computer Science",
        "year": "Apr 2008 - May 2012",
...
- linkedin must be exactly: "linkedin": "https://www.linkedin.com/in/carlos-capulong/"
...
- Domain language aligned to Carlos's real company contexts plus JD themes
...
- State experience as 14+ years in technology including 12+ years of Salesforce delivery (do not inflate beyond this)
...
- Mention the relevant business domain when supported (public safety communications, Salesforce consulting, telecom CRM, staffing, professional services)
...
It lists EVERY Salesforce cloud and product the JD names, in the JD's order of emphasis, BEFORE Carlos's other clouds. If the JD says "Primary Skill: Service Cloud / Required: Data Cloud, Agentforce, PSS", the row reads:
...
- Include relevant technologies already in Carlos's background even when the JD is silent, but keep them subordinate to JD priorities.
- Do not paste the JD's skill list verbatim as a block and do not keyword-dump. Tier 0 terms belong here; the surrounding stack must still be Carlos's own.
...
- Use the category names from the approved list above. Do not reproduce the generic category set from Carlos's existing resume ("Platform Administration & Security", "Integration & Data", "Analytics", "Technical Leadership") — those are the untailored source, and repeating them is a sign no tailoring happened.
...
- Intrado Life & Safety Inc (May 2023 - Present): 10-12 bullets
- Simplus (Nov 2021 - Apr 2023): 9-11 bullets
- Third Pillar (Sep 2020 - Oct 2021): 8-10 bullets
- Pointwest Squad Inc (Mar 2016 - Aug 2020): 8-10 bullets
- Deloitte (Jun 2012 - Dec 2015): 3-5 bullets

Set "location"
Set "location" for each role to the exact "City, State, Country | Mode" string from FIXED COMPANY HISTORY.
NEVER append an employment type to the location line. Full-Time, Part-Time, Contract, Contract-to-Hire, Temporary, Intern, Freelance, W2, C2C and every variant are banned from the location line only. Work mode (Hybrid / Remote / On-Site) stays. The Deloitte title must stay exactly "Software Engineer Intern / Junior Salesforce Developer" — Intern and Junior are required and must never be stripped.
...
Action verb + specific system, workflow, or business process + exact technology + what Carlos personally owned + engineering or business result.
...
5. What did Carlos personally design, build, configure, integrate, test, migrate, secure, deploy, debug, or document?
...
DEPTH TARGETS
- Intrado Life & Safety Inc / Simplus / Third Pillar / Pointwest Squad Inc: one long sentence each, roughly 150-260 characters, naming a concrete Salesforce artifact.
- Deloitte: roughly 100-170 characters, framed as internship / junior Salesforce foundation work.
...
When the JD names a technology, show depth instead of the bare name, wherever Carlos's history supports it:
...
Use only features consistent with Carlos's actual work and with the technology available during that employment period.

==================================================
DEPTH PLAYBOOK — WHEN THE JD LEANS ON AN AREA
==================================================

INTEGRATION DEPTH. When integrations matter, make them one of the strongest areas of the resume, and for each integration bullet identify as many of these as the history supports: source system, target system, the business data exchanged, the integration technology, the authentication method, the payload format, the sync pattern, the error-handling pattern, and what Carlos personally owned. Real pairings for his history: Salesforce to ERP and billing platforms, to field-service dispatch and scheduling, to student-information and campus systems, to distributor and inventory systems, to marketing automation, and to data warehouses through MuleSoft.
...
NON-SALESFORCE ROLES. Deloitte keeps its internship / junior identity. Connect it forward through requirements support, configuration fundamentals, testing, and documentation. Never restyle it as a senior Salesforce architecture role.
...
Surface real transferable domain experience when the JD belongs to an industry Carlos actually served:
- Public safety and emergency communications: Intrado Life & Safety Inc — 911 and emergency response platforms, public safety routing, enterprise communication workflows, life safety technology, operational systems.
- Salesforce consulting and digital transformation: Simplus — advisory-led Salesforce implementation, CPQ and quote-to-cash, Pardot, data and systems integration, change management, managed services.
- Telecom, banking, insurance, real estate, and healthcare CRM: Third Pillar — CRM, eProcurement, eSignature, data cleansing, hyperautomation; Salesforce and Tableau partner implementations.
- IT staffing and delivery: Pointwest Squad Inc — Salesforce development supporting screened and on-boarded delivery resources.
- Professional services: Deloitte — software engineering internship and junior Salesforce development in Singapore.

Outside of Tier 0,Outside of Tier 0, do not claim industry regulations, clinical systems, or platforms with no factual basis. Never claim a government agency, program, or contract Carlos did not work on, and never claim a clearance.

==================================================
COMPANY CONTEXT (use the real business of each employer)
==================================================

Intrado Life & Safety Inc (Longmont, Colorado, United States | current):
Public safety communications and emergency technology company founded 1979, 1,001-5,000 employees, IT Services and IT Consulting.
Carlos's work: senior Salesforce engineering across emergency-communications CRM — Service Cloud, Apex and Lightning Web Components, integrations, data architecture, automation, reporting, and release support.

Simplus (Salt Lake City, Utah, United States):
Salesforce consulting and digital transformation firm (later under Infosys), 501-1,000 employees. Specialties: Sales Cloud, Community Cloud, CPQ, billing and CLM, quote-to-cash, Pardot, advisory, change management, managed services.
Carlos's work: senior Salesforce developer and admin — client delivery across Sales Cloud and related clouds, configuration and development, CPQ-adjacent quote-to-cash where historically accurate, integrations, UAT, documentation, and production support.

Third Pillar Business Applications, Inc. (Makati, Metro Manila, Philippines):
Technology solutions firm (Globe Business Company) offering CRM, eProcurement, eSignature, data cleansing, and hyperautomation, 51-200 employees.
Carlos's work: senior Salesforce developer and integration engineer — CRM implementations, API integrations, data quality workflows, automation, and industry solutions across telecom and related verticals.

Pointwest Squad Inc (Makati, Metro Manila, Philippines):
IT staffing and workforce fulfillment firm, 11-50 employees.
Carlos's work: Salesforce developer across multi-year delivery — Apex, declarative automation, configuration, reporting, integrations, and production support. Longest early Salesforce tenure — show sustained ownership.

Deloitte (Singapore, Singapore | internship / junior):
Global professional services network, 10,001+ employees, Business Consulting and Services.
Carlos's work: software engineer internship / junior Salesforce developer — requirements support, configuration, Apex and Visualforce fundamentals, testing, documentation. Keep clearly junior.
==================================================
CAREER PROGRESSION AND ANTI-OVER-TAILORING
==================================================

- Intrado Life & Safety Inc carries the strongest JD alignment and senior-engineering voice.
- Simplus and Third Pillar are senior consulting and integration roles.
- Pointwest Squad Inc shows multi-year hands-on Salesforce development.
- Deloitte shows early internship / junior foundation only.
- Never place modern tooling in a historical role before it plausibly existed (no Data Cloud or Agentforce in 2012-2015 Deloitte years).
- Do not make every role contain every JD keyword, do not copy JD sentences, and do not recreate the target company's product as if Carlos already built it.
...
Technical specificity outranks numbers. Include at most 1-2 modest, defensible metric-style results in the Intrado Life & Safety Inc role and optionally one in Simplus or Third Pillar, only when natural. Prefer qualitative outcomes: reduced manual processing, fewer recurring defects, cleaner releases, faster troubleshooting, improved data accuracy, stronger access control, better integration stability. Never claim revenue impact, team size, user counts, or large percentage gains.
...
Email: carlos.capulong.ex@gmail.com
LinkedIn: https://www.linkedin.com/in/carlos-capulong/
Base identity: Senior Salesforce Engineer
Experience framing: 14+ years of overall technology experience, including 12+ years delivering Salesforce solutions across public safety communications, Salesforce consulting, telecom-adjacent CRM, staffing platforms, and professional services.

Education:
National University of Singapore (NUS) | Bachelor of Computing in Computer Science | Singapore, Singapore | Apr 2008 - May 2012
...
Intrado Life & Safety Inc (Longmont, Colorado, United States) — default title: Senior Salesforce Engineer | May 2023 - Present

Simplus (Salt Lake City, Utah, United States) — default title: Senior Salesforce Developer | Admin | Nov 2021 - Apr 2023

Third Pillar (Makati, Metro Manila, Philippines) — default title: Senior Salesforce Developer | Integration Engineer | Sep 2020 - Oct 2021

Pointwest Squad Inc (Makati, Metro Manila, Philippines) — default title: Salesforce Developer | Mar 2016 - Aug 2020

Deloitte (Singapore, Singapore) — default title: Software Engineer Intern / Junior Salesforce Developer | Jun 2012 - Dec 2015

Intrado Life & Safety Inc, Simplus, Third Pillar, Pointwest Squad Inc, and Deloitte have no recorded work mode in FIXED COMPANY HISTORY — write location as city, state/region, country only unless a mode is explicitly listed above.
==================================================
EXISTING TECHNICAL COVERAGE (draw selectively; never dump all of it)
==================================================

Salesforce clouds and products Carlos works across:
Sales Cloud, Service Cloud, Experience Cloud, Data Cloud, Agentforce, Einstein AI, Revenue Cloud, CPQ, CRM Analytics, Salesforce Platform, MuleSoft Integration Cloud.
...
0g2. RECENT-ROLE PROOF — count, per role, how many Intrado Life & Safety Inc bullets and how many Simplus bullets name a required skill. Both counts must be 3 or more, and every scarce required product must appear in at least one recent-role bullet. If either count is short, add bullets until it is met before returning.
0h. NO EMPLOYMENT TYPE IN LOCATION — search every experience location for "Full-Time", "Full Time", "Part-Time", "Contract", "Temporary", "Intern", "Freelance", "W2", "C2C" and remove it from location only. Each location reads "City, State, Country | Mode" and nothing more. Keep title exactly "Software Engineer Intern / Junior Salesforce Developer" for Deloitte.
...
4b. TAILORING PROOF — the skills table and the Intrado Life & Safety Inc bullets are visibly specific to THIS JD, not a restatement of Carlos's existing resume.
5. RESPONSIBILITY — every bullet shows what Carlos personally did.
...
11. INTERVIEW TEST — every Tier 1/2/3 bullet is one Carlos could explain from memory. Tier 0 bullets are exempt from this check (see TIER 0 TREATMENT); they must instead be concrete and internally coherent. Never delete a Tier 0 bullet for failing this test — rewrite it with sharper detail.
